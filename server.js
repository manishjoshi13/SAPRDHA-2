...
app.post("/form", async (req, res) => {
    try {
        ...
        if (!course || typeof course !== "string" || !course.trim()) errors.course = "Course is required";
        if (!year || isNaN(parseInt(year))) errors.year = "Year is required";
        if (!gender || typeof gender !== "string" || !gender.trim()) errors.gender = "Gender is required";
        ...
        if (!sportsArray.length) errors.sports = "At least one sport selection is required";
        ...
        if (partners) {
            if (Array.isArray(partners)) {
                ...
                partnersArray = partners
                    ...
                    .filter(p => p && typeof p === "object" && p.sport && p.name && String(p.sport).trim() && String(p.name).trim())
                    .map(p => ({ sport: String(p.sport).trim(), name: String(p.name).trim() }));
                ...
                Object.entries(partners).forEach(([sportKey, partnerValue]) => {
                    if (partnerValue && typeof partnerValue === "object" && partnerValue.name && partnerValue.name.trim()) {
                        partnersArray.push({
                ...
                Object.keys(yearGroups)
                    ...
                    .forEach(year => {
                        ...
                        yearGroups[year].forEach((player, idx) => {
                            let playerText = `${idx + 1}. ${player.name} (${player.course}, ${player.gender})`;
                            // Partner
